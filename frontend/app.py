...
Simple Streamlit frontend for the multi-agent knowledge assistant.
"""
...
# API call
# ─────────────────────────────────────────────
def call_api(user_input: str, session_id=None) -> dict:
...
# ─────────────────────────────────────────────
# Agent guide in expander
# ─────────────────────────────────────────────
with st.expander("📖 How to use Knowva — Agent Guide"):
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("**📋 Summarizer**")
        st.caption("Paste any text and ask to summarize, condense, or get key points.")
        st.code("Summarize this: [paste text]", language=None)
    ...
    with col2:
        ...
        st.caption(
            "Use 'research', 'look up', or 'find information about' for deep research with Wikipedia and web search."
        )
        st.code("Research the history of the internet", language=None)

    st.info(
        "💡 **Pro tip:** After any QnA answer, click **Go Deeper →** to get a full research-level response with tools."
    )
...
for msg in st.session_state.messages:
    if msg["role"] == "user":
        ...
        with st.chat_message("assistant", avatar=icon):
            # Response text — strip escalation offer line
...
if st.session_state.show_escalation:
    ...
    with col_hint:
        st.caption(
            "Get a comprehensive research-level answer with Wikipedia and web search tools."
        )
...
user_input = st.chat_input(
    "Ask a question, paste text to summarize, or say 'Research...'"
)
...
if user_input and user_input.strip():
    ...
    with st.spinner("Thinking..."):
        result = call_api(user_input.strip(), session_id=st.session_state.session_id)
...
# ─────────────────────────────────────────────
# Sidebar — session info and clear
# ─────────────────────────────────────────────
with st.sidebar:
    st.markdown("### 🔮 Knowva")
